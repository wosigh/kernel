...
static unsigned short rh_product_id[] = {
	[RH_CONFIG_UMS] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x4,
	[RH_CONFIG_UMS_NOVACOM] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x2,
	[RH_CONFIG_PASSTHRU] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x3,
	[RH_CONFIG_USBNET_PASSTHRU] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x5,
	[RH_CONFIG_USBNET_UMS_NOVACOM] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x6,
	[RH_CONFIG_PASSTHRU_NOVACOM] = CONFIG_USB_ROCKHOPPER_PID_BASE + 0x7,
	[RH_CONFIG_SENTINEL] = 0,
};
...
static unsigned int config_num = RH_CONFIG_USBNET_UMS_NOVACOM; /* 5 */
module_param(config_num, uint, S_IRUGO);
MODULE_PARM_DESC(config_num, "config number, default=5 (USBNET+UMS+NOVACOM)");
...
#ifdef USE_NDUID_AS_SERIAL_NUMBER
	if (nduid_string_get()) {
		strncpy(serial_number, nduid_string_get(), sizeof(serial_number)-1);
...
static ssize_t
config_num_show(struct class *class, char *buf)
{
	return sprintf(buf, "%d\n", config_num);
}

static ssize_t
config_num_store(struct class *class, const char *buf, size_t count)
{
	char *endp;
