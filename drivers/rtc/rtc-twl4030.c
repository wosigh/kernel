...
/* 
 * Enables periodic event
 */
static int set_rtc_freq_event(unsigned char event)
{
	unsigned char val;
	int ret;
...
static int get_rtc_time(struct rtc_time *rtc_tm)
{
	...
	/*
	 * Latch the current RTC time.
	 * Explicitly set STOP_RTC_M to make sure we don't stop RTC
	 * accidentally.
	 */
	val = BIT_RTC_CTRL_REG_GET_TIME_M | BIT_RTC_CTRL_REG_STOP_RTC_M;
	ret = twl4030_rtc_write_u8(val, REG_RTC_CTRL_REG);
	if (ret < 0)
		return ret;
 ...
 * reading the periodic interupts. The rtc interface defines freq in term of Hz. 
 * In this terms 1Hz is the only supported freq by this chip.
 * All other periodic events
 * 		0: every second
 * 		1: every minute
 * 		2: every hour
 * 		3: every day
 * 	must be handled through custom IOCTL (if needed)
 */ 
static int twl4030_rtc_irq_set_freq(struct device *dev, int freq)
{
	...
	set_rtc_freq_event(0);

	return 0;
}

#if 0
static int twl4030_rtc_irq_set_event(struct device *dev, int event)
{
	...
	/* set rtc irq event to user defined value */
	set_rtc_freq_event(event);
...
static int twl4030_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	// we want to preserve ALARM interrupt 
	// because we want to use it as wakeup source
	mask_rtc_irq_bit(BIT_RTC_INTERRUPTS_REG_IT_TIMER_M);

	// if alarm interrupt is set configure wakeup
	if( irqstat & BIT_RTC_INTERRUPTS_REG_IT_ALARM_M )
		enable_irq_wake(TWL4030_MODIRQ_PWR);
...
{
	// if there was an alarm interrupt set disable wakeup
	if( irqstat & BIT_RTC_INTERRUPTS_REG_IT_ALARM_M )
		disable_irq_wake(TWL4030_MODIRQ_PWR);
