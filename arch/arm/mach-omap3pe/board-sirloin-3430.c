...
#ifdef CONFIG_VIDEO_VX6852
#include <media/vx6852.h>
#endif

#ifdef CONFIG_VIDEO_VX6953
...
struct board_sbc_cfg {
	...
	.timer_id   = 9,
#ifdef CONFIG_MACH_SIRLOIN_3630
	.pinmux_off = "T8_LCD_LED_PWM_OFFMODE",
	.pinmux_on  = "T8_LCD_LED_PWM",
#else
                              ...
                              int polarity, int width, int yield)
{
	...
	sclk=prcm_get_system_clock_speed();

	counts=(width-10)*sclk/(1<<(ILED_TIMER_PRESCALER+1))/1000;

	if (counts < 1) { 
		counts = 1;
	}
	mcount = (ulong) 0xffffffff - ((long) counts);
...
static void *board_sbc_init(void *config_data)
{
	...
	omap_dm_timer_set_prescaler(sbc_data->pulse_timer, ILED_TIMER_PRESCALER);
...
#ifdef CONFIG_VIDEO_VX6953
static struct vx6953_platform_data board_vx6953_platform_data = {
	.fourcc = V4L2_PIX_FMT_SGRBG10,
	.coarse_integration_time = 320,
	.analogue_gain_code_global = 192,	// 4x
	.vt_sys_clk_div = 1,
	.pre_pll_clk_div = 1,
	.pll_multiplier = 98, /* use even number here */
	.op_sys_clk_div = 1,
	.csi_signaling_mode = 1,
	.enable_vdig = board_cam_vcc_enable,
	.extclk = { // 6.5 Mhz
		.numerator = 13,
		.denominator = 2, 
...
static struct i2c_board_info vx6953_i2c_board_info = {
	I2C_BOARD_INFO(VX6953_I2C_DEVICE, VX6953_I2C_ADDR),
	.platform_data = &board_vx6953_platform_data,
};
#endif // CONFIG_VIDEO_VX6953
...
#ifdef CONFIG_VIDEO_VX6953
	i2c_register_board_info(2, &vx6953_i2c_board_info, 1);
#endif // CONFIG_VIDEO_VX6953
...
static struct omap34xx_isp_platform_data board_omap34xx_isp_data = {
	...
	.ccp2_ctrl_fracdiv = 0x8000,
	.ccp2_lc0_ctrl_format = OMAP34XX_CCP2_LCx_CTRL_FORMAT_RAW8_DPCM10_VP,
	.ccp2_lc0_ctrl_crc_en = 1,
	.ccp2_lc0_stat_size_sof = 2,
	.ccp2_lc0_dat_start_vert = 2,
	.ccp2_lc0_ctrl_dpcm_pred = 1, /* Use simple predictor */
	...
	.smia10_sof_len = ((ISP_BASE_5MP_OUT_WIDTH * 10) / 8) * 2,
	.video_width = 480,
	.video_height = 320,
	.video_fourcc = V4L2_PIX_FMT_UYVY,

	/* 1x capture, 2x stats */
	.dma_pool = PAGE_ALIGN((94208 * 2) +
		ALIGN(ISP_BASE_5MP_OUT_WIDTH * 2, 0x20) * ALIGN(ISP_BASE_5MP_OUT_HEIGHT, 16)),
};
...
static struct user_pin wifi_pins[] = {
	...
	},
	{
		.name       =  "irq",
		.gpio       =  WIFI_IRQ_GPIO,
		.options    =  PIN_READ_ONLY,
		.act_level  =  1, // active high
		.direction  =  1, // an input
		.def_level  =  0, // default level (low)
		.pin_mode   =  WIFI_IRQ_GPIO_PIN_MODE,
		.sysfs_mask =  0444,
	},
};
...
static struct hsdl9100_platform_data sirloin_hsdl9100_platform_data = {
	...
	.ledon_duty     = 50,    /* 50% duty cycle for Castle (plastic).
				    NOTE:
				      For Roadrunner devices with a glass cover
				      we use a different duty cycle. We detect
				      the cover type (glass or plastic) based
				      on a command line variable (HousingA)
				      which is passed in by the bootloader. */
...
#ifdef CONFIG_MACH_SIRLOIN_3630
static int __init housing_a_type(char *str)
{
	/* Detect glass cover based on the HousingA type of the device. This
	 * variable is passed in by the bootloader of the HousingA token exists
	 * in the device.
	 */
	if (!strcmp(str, "glass")) {
		/* 10% duty cycle for Roadrunner (glass) */
		sirloin_hsdl9100_platform_data.ledon_duty = 10;
	}
        return 0;
}
__setup("HousingA=", housing_a_type);
...
static void __init board_mux ( void )
{
...
#ifdef CONFIG_MACH_SIRLOIN_3630
	/* LCD SPI2 */
	omap_cfg_reg ( "AA3_3430_LCD_SPI2_CLK" );
	omap_cfg_reg ( "Y2_3430_LCD_SPI2_O"   );
	omap_cfg_reg ( "Y3_3430_LCD_SPI2_I"   );
	...
	/* WiFi Pins */
#ifdef CONFIG_MACH_SIRLOIN_3630
	omap_cfg_reg ("N28_WL_SD0_CLK");
	omap_cfg_reg ("M27_WL_SD0_CMD");
	omap_cfg_reg ("N27_WL_SD0_D0");
	omap_cfg_reg ("N26_WL_SD0_D1");
	omap_cfg_reg ("N25_WL_SD0_D2");
	omap_cfg_reg ("P28_WL_SD0_D3");

	/*
	 *  gpio_126/gpio_127 have dual voltage IO driver
	 *       bit 9 in PBIAS1 - controls PBIAS cell
	 *       bit 6 in CONTROL_WKUP_CTRL - controls output of driver
	 *  Make eed to make sure that both bits are set
	 */
	omap_cfg_reg ("P27_WL_IRQ");
	omap_cfg_reg ("P26_WL_RST_nPWD");
	if( (CONTROL_WKUP_CTRL & (1 << 6)) == 0 ||
	    (CONTROL_PBIAS_1   & (1 << 9)) == 0 ) {
		CONTROL_WKUP_CTRL   &= ~ (1 << 6);
		CONTROL_PBIAS_1     &= ~((1 << 9)); /* turn it off */
		CONTROL_PBIAS_1     &= ~((1 << 8)); /* set for 1.8V operation */
		udelay(1000);			    /* let it settle */
		CONTROL_PBIAS_1     |=  ((1 << 9)); /* enable it again */
		CONTROL_WKUP_CTRL   |=   (1 << 6);
	}
	omap_cfg_reg ("R27_NC");
	omap_cfg_reg ("R25_NC");
...
#ifdef CONFIG_MACH_SIRLOIN_3630
	omap_cfg_reg ("Y21_GPIO_156_OPTICAL_SLIDER");
#else
...
#ifdef CONFIG_MACH_SIRLOIN_3630
	omap_cfg_reg ("T2_3430_GPIO49");       // Prox sensor INT pin
#else
	omap_cfg_reg ("T8_3430_GPIO55");       // Prox sensor INT pin
#endif
	...
	/* tps6105x flash_sync */
#ifdef CONFIG_MACH_SIRLOIN_3630
	omap_cfg_reg("D25_3430_GPIO_126");
	...
	/* MMC Flash */
#ifdef CONFIG_MACH_SIRLOIN_3630
	omap_cfg_reg ("AE2_MMC2_CLK" );
	omap_cfg_reg ("AG5_MMC2_CMD" );
	omap_cfg_reg ("AH5_MMC2_DAT0");
	omap_cfg_reg ("AH4_MMC2_DAT1");
	omap_cfg_reg ("AG4_MMC2_DAT2");
	omap_cfg_reg ("AF4_MMC2_DAT3");
	omap_cfg_reg ("AE4_MMC2_DAT4");
	omap_cfg_reg ("AH3_MMC2_DAT5");
	omap_cfg_reg ("AF3_MMC2_DAT6");
	omap_cfg_reg ("AE3_MMC2_DAT7");
#else
	...
	omap_cfg_reg ("Y21_GPIO_156"); /* used on sirloin 3630 */
#endif
...
#ifdef CONFIG_MACH_SIRLOIN_3630
MACHINE_START(SIRLOIN, "Sirloin OMAP3630 board")
#else
