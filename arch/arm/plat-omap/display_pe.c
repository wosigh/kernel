...
static void omap2_synclost_isr(void *arg, struct pt_regs *regs)
{
	...
	printk(KERN_WARNING
		"DISPLAY: ***** Sync Lost LCD (DISPC_IRQSTATUS %x) *****\n",
							       status);
	/* If we do not have a SYNC LOST interrupt here, we just clear the
	 * status and return.
	 */
...
static void wait_for_go_bit(void)
{
	uint32_t control;
	...
	for(;;) {
		control = dispc_reg_in(DISPC_CONTROL);
		if (!(control & DISPC_CONTROL_GOLCD)) {
			break;
		}
	}
}
...
void omap2_disp_pre_freq_change(void)
{
	uint32_t value;

	value = dispc_reg_in(DISPC_GFX_ATTRIBUTES);
	value &=  ~DISPC_GFX_ATTRIBUTES_ENABLE;
	dispc_reg_out(DISPC_GFX_ATTRIBUTES, value);

	wait_for_go_bit();

	value = dispc_reg_in(DISPC_CONTROL);
	value&= ~DISPC_CONTROL_LCDENABLESIGNAL;
	dispc_reg_out(DISPC_CONTROL, value);
}


void omap2_disp_post_freq_change(void)
{
	uint32_t value;
