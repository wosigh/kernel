	...
	for (resp = res_list; *resp; resp++) {
		...
		for (i = 0; i < MAX_LEVEL; i++) {
			ATOMIC_INIT_NOTIFIER_HEAD(
					&(*resp)->pre_notifier_list[i]);
			ATOMIC_INIT_NOTIFIER_HEAD(
					&(*resp)->post_notifier_list[i]);
		}
...
/*
 *  Reset target opp to lowest possible state
 */
static unsigned long
reset_target_level (struct  shared_resource *resp)
{
	if( resp->prcm_id == PRCM_VDD1_CONSTRAINT )
		return MIN_VDD1_OPP;

	if( resp->prcm_id == PRCM_VDD2_CONSTRAINT )
		return MIN_VDD2_OPP;

	if( resp->prcm_id == PRCM_ARMFREQ_CONSTRAINT )
		return get_arm_freq_for_opp(MIN_VDD1_OPP)/1000;

	if( resp->prcm_id == PRCM_DSPFREQ_CONSTRAINT )
		return get_dsp_freq_for_opp(MIN_VDD1_OPP)/1000;
...
int resource_release(struct res_handle *res)
{
	...
	if (target_level == DEFAULT_LEVEL) {
		/* no constrains left */
		target_level = reset_target_level ( resp );
	}
...
/* Fixup the current level for a resource */
